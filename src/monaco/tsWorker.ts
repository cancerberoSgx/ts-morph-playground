...
export function getScriptFileNames(): Promise<string[]> {
  return getTsWorker(monaco.editor.getModels().find(m => !m.isDisposed())!.uri)
    .then(worker => {
    ...
    getDefinitionAtPosition(model.uri, model.getOffsetAt(position)).then(defs => {
      if (defs) {
        const locations = defs.map(def => {
          ...
          const range = monaco.Range.fromPositions(targetModel.getPositionAt(def.textSpan.start), targetModel.getPositionAt(def.textSpan.start + def.textSpan.length))
          return {
            uri,
            range
          }
